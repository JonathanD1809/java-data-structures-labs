...
public class RecursiveSearch {
    ...
    public static int recursiveBinarySearch(int[] arr, int key, int low, int high) {
        ...
        // Base Case: If the search range is invalid, the key is not present.// nếu  low > high thì trả về -1
        if (low > high) {// tại sao lại là low > high  vì khi ta chia mảng liên tục thì có thể low sẽ vượt quá high
            return -1;
        }
        ...
        // 3. If key < arr[mid], search the left half.
        else if (key < arr[mid]) {
            return recursiveBinarySearch(arr, key, low, mid - 1);
        }

        // 4. If key > arr[mid], search the right half.
        else {
    ...
    public Node searchBST(Node root, int key) {
        ...
        // 1. If the key is less than the root's key, search the left subtree.
        if (key < root.key) {
            return searchBST(root.left, key);
        }

        // 2. If the key is greater than the root's key, search the right subtree.
        else {
